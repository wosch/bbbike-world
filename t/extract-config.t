...
sub config_success {
    ...
    isnt($option, undef, "option");
    is($Extract::Config::spool->{'confirmed'}, "confirmed", "spool confirmed");
    
    isnt($email_allow_nobody, $option->{'email_allow_nobody'}, "email_allow_nobody changed");
...
sub config_failed {
    my ($q, $conf) = shift;
    our $option2 = clone($conf);
    
    my $homepage = $option2->{'homepage'};
    my $config = Extract::Config->new('q' => $q, 'option'=> $option2);
    $config->load_config(); #'/bbbike-extract.rc');
    is($homepage, $option2->{'homepage'}, "homepage not changed");
    isnt($homepage, $option->{'homepage'}, "homepage not changed");
    isnt($option->{'homepage'}, $option2->{'homepage'}, "homepage not changed");
    
    $config = Extract::Config->new('q' => $q, 'option'=> $option2);
    $config->load_config('/bbbike-extract.rc');
    is($homepage, $option2->{'homepage'}, "homepage not changed");
}
...
sub config_success_pro {
    ...
    isnt($email_allow_nobody, $option->{'email_allow_nobody'}, "email_allow_nobody changed");
    isnt($script_homepage, $option->{'script_homepage'}, "script_homepage changed");
    isnt($script_homepage, "", "script_homepage not empty");
    
    is($spool_dir, undef, "spool_dir not empty");
    isnt($option->{'spool_dir'}, $spool_dir, "spool_dir changed to $option->{'spool_dir'}");
