...
do
  ...
  (
    ...
    if [ $(ls $sub_region.osm.mapsforge-*.zip 2>/dev/null | wc -l) -gt 0 -a $(find $sub_region.osm.mapsforge-*.zip -mtime -${max_days} 2>/dev/null | wc -l) -gt 0 ]; then
      ...
      $debug && echo "java_heap: $java_heap pbf2osm_max_cpu_time=$pbf2osm_max_cpu_time area size: $(du -hs $sub_region.osm.pbf)"
      env OSM_CHECKSUM=false pbf2osm_max_cpu_time=14400 java_heap=$java_heap \
